...
export  function SignalStateDiagnosticOutPut(props: SignalStateDiagnosticOutPutProps) {
  const classTailWind = (() => {
    ...
    return { 
      ...
      fieldset_grid: `  grid grid-cols-4 gap-[2px] place-content-stretch box-border`,      
  ...
  return (
      <article>
        <section class={`mb-4 grid grid-cols-4 gap-[5px] place-content-stretch box-border`}>
            ...
            <p class={`bg-blue-500/50 rounded-b-md  text-center`}>{TREE_LAYERS.value.GEN}</p>
            <p class={`bg-blue-500/50 rounded-b-md  text-center`}>{TREE_LAYERS.value.END}</p>
            <p class={`bg-blue-500/50 rounded-b-md  text-center`}>{TREE_LAYERS.value.GAP}</p>
        </section>
        <section class={`mb-4 grid grid-cols-4 gap-[5px] place-content-stretch box-border`}>
          ...
          {TREE_RHO.value.map((X, I) =>{
            return(
              <>
                ...
                <div class={`bg-[${X.HUE}]/50 rounded-md`}>{X.rPA.toFixed(3)}</div>
                <div class={`bg-[${X.HUE}]/50 rounded-md`}> - </div>
                <div class={`bg-[${X.HUE}]/50 rounded-md`}> - </div>            
                <div class={`bg-[${X.HUE}]/90 rounded-md`}>{`φ-${I}-PE`}</div>
                <div class={`bg-[${X.HUE}]/90 rounded-md`}>{X.rPE.toFixed(3)}</div>
